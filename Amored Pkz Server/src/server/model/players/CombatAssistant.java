...
public class CombatAssistant{
	...
	private int combatExp() {
        int amount = 0;
        if (c.easyMode) {
                c.getPA().addSkillXP((Config.EASY_MODE_EXP_RATE/3), 3);
        } else if (c.mediumMode) {
                c.getPA().addSkillXP((Config.MEDIUM_MODE_EXP_RATE/3), 3);
        } else if (c.hardMode) {
                c.getPA().addSkillXP((Config.HARD_MODE_EXP_RATE/3), 3);
        } else if (c.extremeMode) {
                c.getPA().addSkillXP((Config.EXTREME_MODE_EXP_RATE/3), 3);
        }
                return amount;
        }
	...
	public void attackNpc(int i) {	
	  ...
	  if (c.playerEquipment[c.playerWeapon] == 15241) {
          c.gfx0(2138);
			...
			//polypore and armadyl staff by dr house
	 ...
	 if(c.playerEquipment[c.playerWeapon] == 15241 && 15243 != c.playerEquipment[c.playerArrows]){
		 ...
		 if (c.playerEquipment[c.playerWeapon] == 1391) {
				//c.autocasting = true;
				c.spellId = 52;
				c.usingMagic = true;
				//c.autocasting = false;
			}
		if (c.playerEquipment[c.playerWeapon] == 84) {
				//c.autocasting = true;
				c.spellId = 53;
				c.usingMagic = true;
				//c.autocasting = false;
		...
		if (Server.npcHandler.npcs[i] != null) {
			...
			if(!c.castingMagic && c.projectileStage > 0) { // range hit damage
				...
				if(Misc.random(Server.npcHandler.npcs[i].defence) > Misc.random(10+calculateRangeAttack()) && !ignoreDef) {
					...
					damage = 0;
				} else if (c.playerEquipment[c.playerWeapon] == 15403 && Server.npcHandler.npcs[i].npcType == 2881) {
					damage *= 1.5;      
				}
	...
	**/
		...
		if (c.playerEquipment[c.playerWeapon] == 84) {
		  ...
		  if (!c.autocasting) {
            ...
            for (int u : c.Bolts)  {
                for (int y : c.BOWS)  {
                    if(y == c.playerEquipment[c.playerWeapon] && u == c.playerEquipment[c.playerArrows] && c.playerEquipment[c.playerWeapon] != 4734 && c.playerEquipment[c.playerWeapon] != 9185 && c.playerEquipment[c.playerWeapon] != 18357 && c.playerEquipment[c.playerWeapon] != 15241){
	...
	if(!usingCross && !usingArrows && usingBow && (c.playerEquipment[c.playerWeapon] < 4212 || c.playerEquipment[c.playerWeapon] > 4223)) {
			...
			if (c.soulSplitDelay <= 1) {
				applySoulSplit(i, 1);//Maded by zant!!
				}
								if(c.curseActive[10]) { //Leech Attack
...
public void appendMultiBarrage(int playerId, boolean splashed) {
	...
	public void applyPlayerClawDamage(int i, int damageMask, int damage){
		...
		applySmite(i, damage);
        if (c.soulSplitDelay <= 1) {
            applySoulSplit(i, damage);//Maded by zant
        }
switch(damageMask) {
			case 1:
...
break;
			...
			case 20061: // abby whip
			if(Server.npcHandler.npcs[i] != null) {
				Server.npcHandler.npcs[i].gfx100(453);
			}
			c.specAccuracy = 99.99;
			c.startAnimation(1914);
			c.hitDelay = getHitDelay(c.getItems().getItemName(c.playerEquipment[c.playerWeapon]).toLowerCase());
			c.forcedText = "TASTE THE WRATH!";
			c.forcedChatUpdateRequired = true;
			c.updateRequired = true;
			break;
...
break;
	...
	**/
	...
	public int getWepAnim(String weaponName) {
		...
		if(c.playerEquipment[c.playerWeapon] == 20062) {
			switch(c.fightMode) {
				case 0:
				return 386;			
				case 1:
	...
	**/
	...
	public int calculateMeleeMaxHit() {
		...
		if (c.playerEquipment[c.playerWeapon] == 4718 && c.playerEquipment[c.playerHat] == 4716 && c.playerEquipment[c.playerChest] == 4720 && c.playerEquipment[c.playerLegs] == 4722) {	
				maxHit += ((c.getPA().getLevelForXP(c.playerXP[3]) - c.playerLevel[3]) * .01) + 1;
		}
